...
class ConnectionRequestTree_EN:
    def __init__(self):
        ...
        # Create nodes for form submission and exit
        self.form_submission_node = TreeNode('awaiting_form_submission', self.awaiting_form_submission)
        self.exit_node = TreeNode('exit', self.exit_request)

        # Add children nodes (state transitions)
        self.root.add_child('awaiting_form_submission', self.form_submission_node)
        self.form_submission_node.add_child('exit', self.exit_node)
    ...
    def awaiting_form_download(self, user_message, session):
        # Provide the form download link as a clickable HTML link with the download attribute
        form_download_link = "https://www.leco.lk/content/files/applications/LECO-CSC-FO-24.pdf"
        response = self._choose_response([
            f"Please download the form by clicking <a href='{form_download_link}' download>Download Form</a>. Once you have filled it out, please submit it.",
            #f"You can download the form by clicking <a href='{form_download_link}' download>Download Form</a>. After filling it out, submit it to proceed.",
            f"To proceed, download the form by clicking <a href='{form_download_link}' download>Download Form</a> and submit it once completed."
        ])
        session['current_state'] = 'awaiting_form_submission'  # Transition to the next state
        update_chat_history(session, "bot", response)
        return self._split_message(response)

    def awaiting_form_submission(self, user_message, session):
        # Confirm form submission
        response = self._choose_response([
            "Thank you for Download the form. After Filling it out, please bring it to nearest branch .",
          
            "Thank you for submitting the form. Please bring the filled form to the nearest branch.",
        ])
        session['current_state'] = 'exit'  # Transition to exit state
        update_chat_history(session, "bot", response)
        return self._split_message(response)
    ...
    def reset_connection_request(self, session):
        ...
        response = "Please download the form to request a new connection."
        update_chat_history(session, "bot", response)
        return self._split_message(response)
